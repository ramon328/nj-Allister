...
import { useFinePointer, useSpotlight } from "../use-motion";
import { gsap, ScrollTrigger, SplitText, reduced } from "../motion";
...
export function Collections({ collections: db }: { collections?: CollectionView[] }) {
  const cards: Collection[] = db && db.length
    ? db.map((c, i) => {
        const base = collections.find((x) => x.href.includes(`c=${c.slug}`)) ?? collections[Math.min(i, collections.length - 1)];
        return { ...base, id: (base?.id ?? "gen") as Collection["id"], href: `/coleccion?c=${c.slug}`, img: c.image_url ?? base.img, alt: c.name, badge: c.badge ?? base.badge, kicker: `${c.tagline ?? ""} · ${c.product_count} modelos`, title: c.name, text: c.description ?? base.text };
      })
    : collections;
  return (
    <section className="section collections" id="colecciones">
      ...
      <div className="container">
        ...
        <div className="bento">
          {cards.map((c, i) => <CollectionCard key={c.href} c={c} index={i} />)}
        </div>
      </div>
    </section>
  );
...
export function Footer() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const page = document.querySelector(".page");
      // Solo en la portada (hero fijo); en páginas internas el footer se muestra sin scrub.
      if (!page || !document.querySelector(".hero-wrap")) return;
